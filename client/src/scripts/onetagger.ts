...
class OneTagger {
    ...
    url(url: string) {
        // In server mode the UI is already in a browser, so open the link
        // here. Asking the backend means calling `webbrowser` on the machine
        // running the server, which in a container has none -- and answers
        // "No valid browsers detected" to a user whose browser plainly works.
        //
        // The desktop build still goes through the backend: there the UI is a
        // webview and window.open does not reach the real browser.
